...
public class BoxGizmaZ : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (ActionMouve) 
		{
			...
			if (plane.Raycast(ray, out rayDistance)) 
			{
				...
				if (Plus) {
					if (positionPointMouse[2] < gameObject.transform.parent.transform.position.z + 10) positionPointMouse[2] = gameObject.transform.parent.transform.position.z + 10;
				} else {
					if (positionPointMouse[2] > gameObject.transform.parent.transform.position.z - 10) positionPointMouse[2] = gameObject.transform.parent.transform.position.z - 10;
				}
				gameObject.transform.position = positionPointMouse;
				gameObject.transform.parent.SendMessage("ActionZ", Mathf.Abs(gameObject.transform.localPosition.z));
			}
		}
	}
}
