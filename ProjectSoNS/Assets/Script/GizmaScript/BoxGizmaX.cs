...
public class BoxGizmaX : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (ActionMouve) 
		{
			...
			if (plane.Raycast(ray, out rayDistance)) 
			{
				...
				if (Plus) {
					if (positionPointMouse[0] < gameObject.transform.parent.transform.position.x + 10) positionPointMouse[0] = gameObject.transform.parent.transform.position.x + 10;
				} else {
					if (positionPointMouse[0] > gameObject.transform.parent.transform.position.x - 10) positionPointMouse[0] = gameObject.transform.parent.transform.position.x - 10;
				}
				gameObject.transform.position = positionPointMouse;
				gameObject.transform.parent.SendMessage("ActionX", Mathf.Abs(gameObject.transform.localPosition.x));
			}
		}
	}
}
